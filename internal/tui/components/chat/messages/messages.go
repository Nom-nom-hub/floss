...
// EnhancedCopyKey is the key binding for copying message content to the clipboard in enhanced messages.
var EnhancedCopyKey = key.NewBinding(key.WithKeys("c", "y", "C", "Y"), key.WithHelp("c/y", "copy"))

// EnhancedClearSelectionKey is the key binding for clearing the current selection in enhanced chat interface.
var EnhancedClearSelectionKey = key.NewBinding(key.WithKeys("esc"), key.WithHelp("esc", "clear selection"))
